...
describe("#updateCoins USD", function () {
  ...
  describe("coinmarketcap.com @e2e", function (done) {
    ...
    let liveData
    before( async () => {
      liveData = await coinmarketcap.ticker({convert: 'USD'})
    ...
    it("results parsed", function(done) {
      ...
      for(let valuation of liveData) {
        mutations.push( ValidationHelpers.ValuationMutationGenerator(valuation.symbol,"USD") )
      }
      mutations.push( ValidationHelpers.LastFetchedMutationGenerator("USD") )
      mutations.push( ValidationHelpers.ToggleLoadingMutationGenerator() )
