...
describe("#updateCoins EUR", function () {
  ...
  describe("coinmarketcap.com @e2e", function (done) {
    ...
    let liveData
    before( async () => {
      liveData = await coinmarketcap.ticker({convert: 'EUR'})
    ...
    it("results parsed", function(done) {
      ...
      for(let valuation of liveData) {
        mutations.push( ValidationHelpers.ValuationMutationGenerator(valuation.symbol,"EUR") )
      }
      mutations.push( ValidationHelpers.LastFetchedMutationGenerator("EUR") )
      mutations.push( ValidationHelpers.ToggleLoadingMutationGenerator() )
